...
void Key_Scan(void)
{
    ...
    for (uint8_t i = 0; i < Key_Number; i++) {                                            // 遍历所有按键
        ...
        if (pinState == PRESSED)                                                          // 按键按下
        {
            if (KeyManager.Key[i].Time_Counter <= KeyManager.keyLongPressTime || KeyManager.Key[i].doubleClickFlag == 1) // 未达到长按时间或正在检测双击
            {
        ...
        } else if (pinState == RELEASED && KeyManager.Key[i].State == Key_Pressed && KeyManager.Key[i].doubleClickFlag != 1 && KeyManager.Key[i].longPressFlag != 1) // 按键释放且未标记为双击或长按,延时检测双击
        {
            KeyManager.Key[i].duobleClick_Counter++; // 增加双击计数器
        ...
        } else if (pinState == RELEASED) {                 // 按键释放
            if (KeyManager.Key[i].doubleClickFlag == 1) {  // 清除双击标志与动作
